...
var BugDispatch = {
    ...
    options: {
        ...
        maxSmallTurnDeg: 10,
        maxWiggleDeg: 5,
        imageSprite: '/ratatouille/bugs/fly-sprite.png',
        bugWidth: 13,
        bugHeight: 14,
        num_frames: 5,
        zoom: 10,
...
var SpiderController = function() {
    var spiderOptions = {
        imageSprite: '/ratatouille/spider-sprite.png',
        bugWidth: 69,
        bugHeight: 90,
        num_frames: 7,
        canFly: false,
...
var Bug = {
    ...
    setPos: function(top, left) {
        this.bug.top = top || this.random(this.options.edge_resistance, document.getElementById('bugs').clientHeight - this.options.edge_resistance);

        this.bug.left = left || this.random(this.options.edge_resistance, document.getElementById('bugs').clientWidth - this.options.edge_resistance);
    ...
    drawBug: function(top, left) {
        ...
        if (!this.inserted) {
            this.inserted = true;
			document.getElementById("bugs").appendChild(this.bug);
        }
    },
    ...
    flyRand: function() {
        this.stop();
        var landingPosition = {};
        landingPosition.top = this.random(this.options.edge_resistance, document.getElementById('bugs').clientHeight - this.options.edge_resistance);
        landingPosition.left = this.random(this.options.edge_resistance, document.getElementById('bugs').clientWidth - this.options.edge_resistance);
    ...
    flyIn: function() {
        ...
        var side = Math.round(Math.random() * 4 - 0.5),
            ...
            windowX = document.getElementById('bugs').clientWidth,
            windowY = document.getElementById('bugs').clientHeight;
        if (side > 3) side = 3;
        if (side < 0) side = 0;
        var style = {},
            s;
    ...
    bug_near_window_edge: function() {
        ...
        else if (this.bug.top > document.getElementById('bugs').clientHeight - this.options.edge_resistance)
            this.near_edge |= this.NEAR_BOTTOM_EDGE;
        if (this.bug.left < this.options.edge_resistance)
            this.near_edge |= this.NEAR_LEFT_EDGE;
        else if (this.bug.left > document.getElementById('bugs').clientWidth - this.options.edge_resistance)
